...
def create_x3d_bottleneck_block(
    *,
    ...
    """
    # 1x1x1 Conv
    conv_a = nn.Conv3d(in_channels=dim_in,
                       out_channels=dim_inner,
                       kernel_size=(1, 1, 1),
                       bias=False)
    norm_a = (None if norm is None else norm(
    ...
    norm_b = nn.Sequential(
        (nn.Identity() if norm is None else norm(
            num_features=dim_inner, eps=norm_eps, momentum=norm_momentum)),
        se,
    )
    ...
    # 1x1x1 Conv
    conv_c = nn.Conv3d(in_channels=dim_inner,
                       out_channels=dim_out,
                       kernel_size=(1, 1, 1),
                       bias=False)
    norm_c = (None if norm is None else norm(
...
def create_x3d_res_block(
    *,
    ...
    return ResBlock(
        ...
        if dim_in != dim_out and use_shortcut else None,
        branch2=bottleneck(
            dim_in=dim_in,
            dim_inner=dim_inner,
            dim_out=dim_out,
...
def create_x3d_head(
    *,
    ...
    """
    pre_conv_module = nn.Conv3d(in_channels=dim_in,
                                out_channels=dim_inner,
                                kernel_size=(1, 1, 1),
                                bias=False)

    pre_norm_module = norm(num_features=dim_inner,
                           eps=norm_eps,
    ...
    post_conv_module = nn.Conv3d(in_channels=dim_inner,
                                 out_channels=dim_out,
                                 kernel_size=(1, 1, 1),
                                 bias=False)

    if bn_lin5_on:
        post_norm_module = norm(num_features=dim_out,
                                eps=norm_eps,
                                momentum=norm_momentum)
    else:
...
def create_x3d(
    *,
    ...
    # print('over here hey oooooooooo, the stem_dim_in is {} and the stem_dim_out is {}'.format(stem_dim_in,stem_dim_out))
...
#######################################################################################
#Nico's implement.
# The goal is to either to have a pretrained or random initialized model of X3D with pytorch lightning
# In our situation we are interested in X3D_L which means
# clip_length = 16
# crop_size = 312

# We want 3 different heads: aact, subact & act 
# Each one of them have different number of classes (52, 68, 17) 
# Therefore the output of each head must be (52*16, 68, 17)


class X3D_MOMA(pl.LightningModule):
    def __init__(self, hparams):
        super(X3D_MOMA, self).__init__()
        self.hparams=hparams
        self.model = torch.hub.load('facebookresearch/pytorchvideo', self.hparams.x3d_type, pretrained=self.hparams.pretrained)# we replaced the last layers to Identity
        self.model.blocks[5].output_pool=Identity()
        self.model.blocks[5].activation=Identity()
        self.model.blocks[5].proj=Identity()
        self.model.blocks[5].pool.post_act=Identity()
        self.model.blocks[5].pool.post_conv=Identity()
        self.model.blocks[5].pool.pool=Identity()                   # TO DO supress these layers instead of identity
        self.model.blocks[5].dropout=Identity()
        
        # since we want many to many, we have to go back to the 16 frames encoding which right before the blocks[5].pool.pool
        self.many_to_one_pool=nn.AvgPool3d(kernel_size=(16, 10, 10), stride=1, padding=0)
        self.many_to_one_post_conv=nn.Conv3d(432, 2048, kernel_size=(1, 1, 1), stride=(1, 1, 1), bias=False)
        self.many_to_one_post_act=nn.ReLU()
        self.many_to_one_dropout=nn.Dropout(p=0.5, inplace=False)
        
        self.many_to_many_pool=  nn.AvgPool3d(kernel_size=(1, 10, 10), padding=0)
        self.many_to_many_post_conv=nn.Conv3d(432, 2048, kernel_size=(1, 1, 1), stride=(1, 1, 1), bias=False)
        self.many_to_many_post_act=nn.ReLU()
        self.many_to_many_dropout=nn.Dropout(p=0.5, inplace=False)
        
        # defining the different fc for the different classes, TO DO rebuild to have 2048 as output size of conv
        # act
        self.act_fc=nn.Linear(in_features=2048, out_features= hparams.activity_out_class) #BX17
        #subact
        self.subact_fc=nn.Linear(2048, hparams.subact_out_class) #BX68
        #aact
        self.aact_fc=nn.Linear(2048, hparams.aact_out_class) # (B*input_clip_length)x53
        self.softmax=nn.Softmax(dim=1)  
        print("done init model")


    def forward(self, x):
        # print('shape of the input is {}'.format(x.shape))
        current_batch_size=x.shape[0]
        x = torch.transpose(x, 1, 2) # number of channels in second place for shape
        out_stem = self.model(x)
        # print('shape of the out_stem is{}'.format(out_stem.shape))
                
        #first many to one
        one=self.many_to_one_pool(torch.reshape(out_stem,(current_batch_size,432,
                                    self.hparams.input_clip_length,10,10)))
        one=self.many_to_one_post_conv(one)
        one=self.many_to_one_post_act(one)
        one=self.many_to_one_dropout(one)
        #second many to many
        many=self.many_to_many_pool(torch.reshape(out_stem,(current_batch_size,432,
                                    self.hparams.input_clip_length,10,10)))
        many=self.many_to_many_post_conv(many)
        many=self.many_to_many_post_act(many)
        many=self.many_to_many_dropout(many)
         
        # now applying the last fc layer respectively many to many or many to one

        act_logits = self.act_fc(torch.reshape(one,(current_batch_size,2048)))  # TO DO use x.shape[0] for the reshape 
        subact_logits = self.subact_fc(torch.reshape(one,(current_batch_size,2048)))
        aact_logits = self.aact_fc(torch.reshape(many,(current_batch_size*self.hparams.input_clip_length,2048)))
        # need to reshape to the 16 frames and 52 activities shape
                
        # aact_prob=self.softmax(aact_logits)
        # subact_prob= self.softmax(subact_logits)
        # act_prob=self.softmax(aact_logits)

        return act_logits , subact_logits , aact_logits

    @staticmethod
    def add_model_specific_args(parser):  # pragma: no cover
        x3d_model_specific_args = parser.add_argument_group(
            title='swin specific args options')
        x3d_model_specific_args.add_argument("--prediction_mode",
                                             default='last_label')
        return parser